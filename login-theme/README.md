...
Three Keycloak 26.x login themes, packaged as a single provider JAR:
...
- **`DiggDs`** is identity-provider selection only, no username/password form, styled after DIGG's
  ...
  rather than `SwedenconnectIdpPsw`'s filled button).

All three extend the stock `keycloak.v2` login theme and override only what differs. Every other
...
`SwedenconnectDS` and `DiggDs` share the same identity-provider list and the same alias-to-icon
matching (see below); they differ only in visual style, so pick whichever look fits the realm.
...
service at <https://iam.digg.se/ds>. The header keeps the Sweden Connect logo, for brand consistency
with the other two themes. Each card also shows a brand icon; see **Identity-provider icons** below.

Dark mode is turned off, since the discovery service has no dark variant.

## Identity-provider icons

`SwedenconnectDS` and `DiggDs` both show a brand icon per identity provider, chosen by matching the
provider's **alias** in the Admin Console against a fixed set of substrings (case-insensitive), in
`idpIconFile()` in each theme's `login.ftl`:
...
placeholder mark, not any provider's logo. The two themes keep separate, identical copies of the icon
files, since Keycloak resource directories are not shared between themes.
...
| File | Purpose |
| :--- | :--- |
| `META-INF/keycloak-themes.json` | Tells Keycloak that the JAR holds the themes `SwedenconnectIdpPsw`, `SwedenconnectDS` and `DiggDs`, all of type `login`. |
...
| `theme/DiggDs/login/resources/img/` | The Sweden Connect logo (header) and `img/idp/`, the bundled provider icons. |
...
2. Set **Login theme** to `SwedenconnectIdpPsw`, `SwedenconnectDS` or `DiggDs`.
3. Save.
...
For `SwedenconnectDS` and `DiggDs`, the identity providers must already be configured under
**Identity providers**; aliases that match the patterns in `idpIconFile()` (see above) get their real
icon, others get the generic fallback.
...
mounted at `/opt/keycloak/themes/<theme-name>/login` (`SwedenconnectIdpPsw`, `SwedenconnectDS` or
`DiggDs`). The Docker Compose file already mounts `compose/config/keycloak/themes` there, and starts
Keycloak with theme caching turned off, so a reload shows the change. Remove that copy again before
using the JAR, since two themes with the same name conflict.

## A note on stability

Each `login.ftl` is a copy of the `login.ftl` of `keycloak.v2`, with the changes described above.
`SwedenconnectIdpPsw` relies on the macros and variables of that theme (`template.ftl`, `field.ftl`,
`buttons.ftl` and `passkeys.ftl`); `SwedenconnectDS` and `DiggDs` rely only on `template.ftl`, since
neither has a form to submit. Keycloak does not treat any of these as a stable API. All three themes
were written against the `keycloak.v2` login theme of Keycloak 26.7.3, the version the Compose file
runs. On every Keycloak upgrade, compare each `login.ftl` with the `login.ftl` of the new `keycloak.v2`
theme and bring over what has changed.
